        ...
        contents: [
          {
            role: 'user',
            parts: [
              { text: `${config.system_prompt}\n\nคำแนะนำสำคัญสำหรับการตอบ:\n1. หากในองค์ความรู้มีหัวข้อ 'รายการคำถาม-คำตอบที่พบบ่อย (FAQ Pairs)' ที่ตรงกับสิ่งที่ผู้ใช้ถาม ให้นำคำตอบที่ระบุในคู่นั้นมาตอบผู้ใช้โดยตรง\n2. หากคำถามเกี่ยวข้องกับสภาพอากาศ ให้นำข้อมูลสภาพอากาศจริงของ อ.ฝาง จ.เชียงใหม่ มาตอบอย่างสุภาพและแม่นยำ\n3. กฎสำคัญ: ห้ามแสดงตัวอักษรนำหน้า เช่น 'Q:', 'A:', 'Q1:', 'A1:', 'คำถาม:', 'คำตอบ:' ในคำตอบอย่างเด็ดขาด\n4. กฎเข้มงวดป้องกันการตอบผิด (Strict Anti-Hallucination): ตอบเฉพาะข้อมูลที่มีระบุอยู่ในเอกสารอ้างอิงเท่านั้น หากในเอกสารไม่มีคำตอบสำหรับสิ่งที่ผู้ใช้ถาม (เช่น ถามราคา หรือถามเรื่องที่ไม่มีในเอกสาร) ให้ตอบว่า "ขออภัยครับ/ค่ะ ยังไม่พบข้อมูลเรื่องที่สอบถามในระบบฐานความรู้ของวิทยาลัยการอาชีพฝาง สามารถติดต่อสอบถามเพิ่มเติมได้ที่งานบริหารงานทั่วไปครับ" และห้ามเดา นำชื่อบุคคล หรือโครงสร้างองค์กรที่ไม่เกี่ยวข้องมาตอบเด็ดขาด\n5. ตอบเป็นข้อความบรรยายภาษาไทยที่สุภาพ เป็นธรรมชาติ และกระชับตรงประเด็น\n\n## องค์ความรู้อ้างอิงจากฐานข้อมูลวิทยาลัยการอาชีพฝาง (Google Sheets Knowledge Base):\n${contextText}${weatherContext}\n\n## คำถามของผู้ใช้:\n${question}` }
            ]
          }
        ],
        generationConfig: {
    ...
    try {
      ...
      const res = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          temperature: config.temperature,
          messages: [
            { role: 'system', content: `${config.system_prompt}\n\nคำแนะนำ: หากมี FAQ Pairs หรือข้อมูลสภาพอากาศ ให้นำมาตอบ ห้ามแสดงสัญลักษณ์ Q: A:\n\n## องค์ความรู้อ้างอิง:\n${contextText}${weatherContext}` },
