...
dataRows.forEach((r, idx) => {
  const email = r[1] || r[24];
  const name = r[2] || r[23] || 'ผู้ใช้งาน';
  const dept = r[5];
  ...
  if (!userSet.has(key)) {
    userSet.set(key, {
      name,
      email: email || `${name.toLowerCase().replace(/[^a-z0-9]/g, '')}@fang.ac.th`,
      dept,
      subDept,
      count: 1
    });
  } else {
    userSet.get(key).count++;
  }
});

console.log('👥 Real Users Extracted from Google Sheet:');
userSet.forEach(u => {
  console.log(`- ${u.name} (${u.email}) [${u.dept} > ${u.subDept}] — ${u.count} records`);
});
